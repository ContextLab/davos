...
In this section, we have contrived a more complex scenario to highlight some of Davos's more advanced features, and illustrate how they may be used to handle certain challenges that can arise when writing and sharing reproducible scientific code.
...
\texttt{scikit-learn}~\cite{PedrEtal11} libraries may be loaded in (using older
...
of an IPython notebook, the code in Figure~\ref{fig:illustrative-example}
...
After installing and importing Davos (lines 1--2), we first use the \texttt{davos.require\_python()} function to constrain the Python version used to run the code below (see Sec.~\ref{subsec:toplevel}).
As described above, the example code in Figure \ref{fig:illustrative-example} uses two different versions of the \texttt{pandas} library: first, an older one needed to access a dataset saved in an outmoded format, then a newer one throughout the remainder of the notebook.
Line 6 therefore ensures upfront that the notebook's Python version falls within the overlap between the ranges of Python versions that these two versions of \texttt{pandas} support. If it does not, the function in line 6 will raise an error that includes a message to this effect (lines 4--5).
\begin{center}
...
module. Since standard library modules are included with all Python
distributions, this line is functionally equivalent to an
\texttt{import} statement and does not need or benefit from an onion
comment. Line 9 loads the \texttt{joblib} package~\cite{Varo10},
...
onion comment in line 9 helps ensure the analysis notebook continues
...
scenario, in lines 26 and 28, we use the \texttt{pip\_executable} option to control which environments each
...
\todo{mention notebook reproducibility, cell order, multiple package versions re: reviewer's comment; note importance of running lines 14--19 \& 38--40 in single cell; say another option is to alias one version, e.g., \texttt{smuggle pandas as pd\_old}}
