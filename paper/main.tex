...
%For example, sharing an analysis or tutorial that relies on a particular Docker image to run properly not only necessitates writing and distributing extra configuration files and setup instructions, but more significantly, requires both the original author and anyone else who may want to run their code to first be able to install and be able to use additional software that is likely more complex and resource-intensive than the actual code it facilitates running.
