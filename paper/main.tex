...
%To prevent Davos-enhanced notebooks from having unwanted side-effects on the user's environment, Davos automatically isolates packages installed via \texttt{smuggle} statements using a custom, virtual environment-like system called ``projects.''
%Functionally, Davos projects are similar to standard Python virtual environments (e.g., created with the standard library's \texttt{venv} module or a third-party tool like \texttt{virtualenv}~\cite{BickEtal07}), but are lighter-weight and simpler to use. 
%Like a standard virtual environment, a Davos project consists of a directory (within a hidden \texttt{.davos} folder in the user's home directory) that houses third-party packages needed for a particular Python-based project \textcolor{red}{[confusing reuse of ``project''?]}, workflow, or task. 
%However, unlike standard virtual environments, Davos projects do not need to be manually created, activated, or deactivated; do not contain separate Python or \texttt{pip} executables; and function to \textit{extend} the user's main Python environment rather than replace it.
...
%Davos projects are similar to standard Python virtual environments (e.g., created with the standard library's \texttt{venv} module or a third-party tool like \texttt{virtualenv}~\cite{BickEtal07}): a project consists of a directory (within a hidden \texttt{.davos} folder in the user's home directory) that houses third-party packages needed to run a particular Python program, workflow, or task. However, unlike standard virtual environments, Davos projects do not need to be manually created, activated, or deactivated, and 
%%function to \textit{extend} 
%serve to \textit{supplement}
%the user's existing Python environment rather than replace it.
...
%Davos will also check for smuggled packages in the notebook's project directory before searching the user's main environment such that project-installed packages are always prioritized when available.
During each \texttt{smuggle} statement's execution, Davos also temporarily prepends the notebook's project directory to the module search path so that these project-installed packages are visible when searching for smuggled packages locally, and given priority over packages in the user's main environment.
%and prioritized over packages in the user's main environment when they are available.

% Thus, rather than constructing a fully separate Python environment from scratch, a Davos project works by providing any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook.

Thus, rather than constructing fully separate Python environments from scratch, Davos projects work by providing any additions or modifications to the user's existing package environment needed to satisfy the dependencies of their corresponding notebooks. In some cases, this might include every package smuggled into a notebook (e.g., if the notebook is run inside a freshly created, empty virtual environment). In other cases, the existing environment might provide all required packages already, and the notebook's project directory will be unused (in which case it will be deleted automatically when the notebook kernel is shut down). But in all cases, any packages installed are confined to the project directory and the user's existing environment will remain unaltered. 



%Thus, rather than creating fully independent Python environments created from scratch, Davos projects provide any additions or modifications to the user's existing package environment needed to satisfy the dependencies of their corresponding notebooks. In some cases, this may be the full set of packages smuggled into a notebook (e.g., if the notebook's runtime environment is a freshly created, empty virtual environment). In other cases, the user's existing environment may 




%Thus, rather than creating a fully separate Python environment from scratch, a Davos project provides any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook---and does so without 


%In some cases... In other cases... In all cases, existing environment unaffected.


%Thus, rather than a fully separate Python environment created from scratch, a Davos project provides any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook---without affecting the existing environment itself.

%Thus, rather than complete Python environments recreated from scratch, Davos projects provide any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook --- and does so without altering the existing package environment.
%
%Thus, rather than a fully separate Python environment created from scratch, a Davos project provides any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook --- and does so without altering the existing package environment
%
%
%Thus, rather than recreating a complete Python environment from scratch, a Davos project provides any additions or modifications to the user's existing package environment needed to satisfy the dependencies of a particular notebook --- without altering the existing package environment. In some cases, 
%
%Thus, a Davos project provides any additions and modifications to the user's existing package environment needed to satisfy 
...
Because \texttt{smuggle} statements are evaluated each time the notebook is run, this system ensures that all required packages and specified package versions are always available and used, even if the notebook's main runtime environment is changed. For example, if a user 
