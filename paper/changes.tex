...
%DIF ADD main.tex   Thu Oct 26 19:50:42 2023
\usepackage{amssymb}
\usepackage{lineno}
\usepackage{float}
\restylefloat{table}
...
the user's existing set of packages. \textbf{Virtual environments} \DIFaddbeginFL \DIFaddFL{(along with
dependency specification files) }\DIFaddendFL similarly allow users to install required
packages into an isolated directory, and \DIFdelbeginFL \textbf{\DIFdelFL{shareable Python environments}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Python environments}} \DIFaddFL{(again,
along with dependency specification files) }\DIFaddendFL extend this to include a specific
version of Python itself. However, both of these systems require users to
create, populate, and manage environments manually. This typically entails
distributing a configuration file (e.g., a \texttt{requirements.txt},
