...
From then on, any further changes to the user's \texttt{Numpy} installation would have no effect on Davos's behavior in this particular notebook, as a satisfactory version now exists in its project directory. (If the version specified in the onion comment were changed, Davos would install it in the project directory in place of the previous version.) 
...
However, if desired, adding \texttt{pip}'s \texttt{--ignore-installed} flag to an onion comment in the notebook will cause the smuggled package to be installed into the project directory whether or not it already exists locally.
