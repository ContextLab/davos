...
allow users to isolate installed packages in a specific directory, and can be
populated with packages from a shared dependency specification
file (e.g., a \texttt{requirements.txt} or \texttt{pyproject.toml~\cite{CannEtal16})}.
\textbf{Python environments} (again, shared in the form of a dependency
specification file; e.g., an \texttt{environment.yml}) extend this idea to allow
installing a specific version of Python itself. However, both
virtual environments and Python environments require users to create, populate,
...
environment. They incorporate (i.e., automatically install) a copy of Python and the target software's
...
notebook. And like separate dependency specification files that list packages
for populating virtual environments or Python environments, \texttt{Davos}
...
Finally, neither specification files used to install packages into virtual environments nor \texttt{smuggle} statements and onion comments included \texttt{Davos}-enhanced notebooks can be used to automatically install a specific version of Python with which to run shared code. 
...
We note that a similar a similar constraint can be imposed by setting the ``\texttt{requires-python}'' attribute in a \texttt{pyproject.toml} file, though creating a virtual environment from a \texttt{pyproject.toml} would require installing an additional third-party tool (such as~\cite{Eust19}).
