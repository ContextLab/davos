...
% It's possible that in some cases, a version mismatch like this could introduce certain compatibility issues, depending on the specific package and the differences between the two versions being used. For example, a function from a newer package version could fail to handle an object from an older version passed to it as an argument, if it expects the object to have certain attributes that had not yet been added in the older version. Fortunately, with \texttt{davos}, this scenario is relatively easy to handle, as \texttt{smuggle} statements can be used to temporarily switch between package versions as needed (e.g., for a single line, in order to call an older version of the function). And because the installer program caches previously installed package versions, switching between package versions in this way can be done fairly quickly.
