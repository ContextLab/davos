...
impose a substantial burden on the user and creates room for mistakes
...
\caption{\small \textbf{Package structure.} The \texttt{davos} package
  ...
  points to a file that imports objects defined or specified at the
  arrow's source).}
...
\item \texttt{.auto\_rerun}: This attribute controls how
  ...
  and if the code that generates those objects was changed between the
...
\texttt{davos} is first imported, or when it is activated after being
...
After importing \texttt{davos} (line 1), we first smuggle two
...
comment. Line 4 loads the \texttt{joblib} package~\cite{Varo10},
...
the data to be read in (and, in line 12, saved out to a \texttt{.csv}
...
After re-enabling these two options (line 19), we next smuggle
...
scenario, on lines 25 and 27 we control which environments each
...
\texttt{sklearn.decomposition.\_lda}.  However, in version
0.22.0, that module was renamed to \texttt{\_online\_lda}, and in
versions 0.22.1 and higher, the name was reverted back to
\texttt{\_lda}.

In order to correctly load the model that includes the pretrained
\texttt{LatentDirichletAllocation} instance, in line 33, we first
...
package version (line 34), we upgrade to a newer version of
\texttt{scikit-learn} in line 35.  Taken together, the code in
