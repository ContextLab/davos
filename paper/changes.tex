...
%DIF DEL old.tex    Thu Oct 26 14:34:06 2023
%DIF ADD main.tex   Thu Oct 26 22:03:50 2023
\usepackage{amssymb}
\usepackage{lineno}
\usepackage{float}
\restylefloat{table}
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.65\textwidth]{figs/shareable_code_2d}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{figs/shareable_code_2d} \DIFaddendFL \caption{\small
\DIFdelbeginFL \textbf{\DIFdelFL{Systems for sharing code within the Python
  ecosystem.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Systems for ensuring code reproducibility within the Python ecosystem.}}
\DIFaddendFL The $x$-axis denotes the ``burden'' placed on users to install and configure
...
the user's existing set of packages. \textbf{Virtual environments} similarly
allow users to \DIFdelbeginFL \DIFdelFL{install required }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{isolate installed }\DIFaddendFL packages \DIFdelbeginFL \DIFdelFL{into an isolated
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in a specific }\DIFaddendFL directory, and \DIFdelbeginFL \textbf{\DIFdelFL{shareable Python environments}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{can be
populated with packages from a shared dependency specification
file~\mbox{%DIFAUXCMD
\citep[e.g., a requirements.txt or pyproject.toml; ][]{CannEtal16}}\hskip0pt%DIFAUXCMD
.
}\textbf{\DIFaddFL{Python environments}} \DIFaddFL{(again, shared in the form of a dependency
specification file; e.g., an environment.yml) }\DIFaddendFL extend this \DIFaddbeginFL \DIFaddFL{idea }\DIFaddendFL to \DIFdelbeginFL \DIFdelFL{include
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{allow
automatically installing }\DIFaddendFL a specific version of Python itself. However, both of
\DIFdelbeginFL \DIFdelFL{these systems }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{virtual environments and Python environments }\DIFaddendFL require users to create, populate,
and manage environments manually. This typically entails distributing a
configuration file (e.g., a \texttt{requirements.txt},
\texttt{pyproject.toml}~\cite{CannEtal16}, or \texttt{environment.yml} file)
that specifies all project dependencies (including version numbers) alongside
the primary code base. Users can then install a third-party
tool~\cite[e.g.,][]{BickEtal07, Eust19, Anac12} to read the file and build the
environment. \textbf{Containers} provide a means of defining an isolated
environment that includes a complete operating system (independent of the
user's operating system), in addition to (optionally) specifying a virtual
environment or other configurations needed to provide the necessary computing
environment. Containers are typically defined using specification files (e.g.,
a plain-text \texttt{Dockerfile}~\cite{Merk14}) that instruct the
virtualization engine regarding how to build the containerized environment.
\textbf{Virtual machines} provide a complete hardware-level simulation of the
computing environment. In addition to simulating specific hardware, virtual
machines (typically specified using binary image files) must also define
operating system-level properties of the computing environment. $\Delta_1$
...
despite more stringently ensuring reproducibility.} \label{fig:code-sharing}
\end{figure}
...
environment. They incorporate \DIFaddbegin \DIFadd{(i.e., automatically install) }\DIFaddend a copy of Python and the target software's
...
different users. However, each of these systems \DIFdelbegin \DIFdel{also }\DIFdelend \DIFaddbegin \DIFadd{either }\DIFaddend relies on additional
software that can be complex or resource-intensive to install and use, \DIFaddbegin \DIFadd{or
requires additional setup, }\DIFaddend creating potential barriers to both contributing to
and taking advantage of open science resources. \DIFaddbegin \DIFadd{For example, what is typically
``shared'' with the end user in these cases is a set of instructions or
configuration files for }\textit{\DIFadd{building}} \DIFadd{the desired computing environment,
...
notebook. And like \DIFdelbegin \DIFdel{virtual environments}\DIFdelend \DIFaddbegin \DIFadd{separate dependency specification files that list packages
for populating virtual environments or Python environments}\DIFaddend , \texttt{Davos}
provides a convenient mechanism for fully specifying (and installing, as
needed) a complete set of Python dependencies, including specific package
versions, which are contained and isolated from the rest of the user's system.
...
While the \texttt{Davos} package by no means offers a universal solution to this problem, it can, in many cases, provide an effective yet more accessible alternative to existing systems for sharing reproducible Python code, such as \DIFaddbegin \DIFadd{specification files for building }\DIFaddend virtual environments, \DIFaddbegin \DIFadd{Python environments, }\DIFaddend containers, and virtual machines (Fig.~\ref{fig:code-sharing}).
